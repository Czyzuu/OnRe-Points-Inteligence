...
test('calculates every strategy formula',()=>{const i={investmentUsd:100,onycPrice:2,holdMultiplier:1,supplyMultiplier:3,lpMultiplier:2,qualifyingShare:.5,leverage:2,loopMultiplier:3,ytPerOnyc:10,ytMultiplier:4,ratexMultiplier:5,vaultMultiplier:4,juniorMultiplier:6,seniorMultiplier:1,customDailyPoints:77};assert.equal(strategyDailyPoints('hold',i),50);assert.equal(strategyDailyPoints('supply',i),150);assert.equal(strategyDailyPoints('lp',i),50);assert.equal(strategyDailyPoints('loop',i),300);assert.equal(strategyDailyPoints('yt',i),2000);assert.equal(strategyDailyPoints('ratex',i),250);assert.equal(strategyDailyPoints('vault',i),200);assert.equal(strategyDailyPoints('junior',i),300);assert.equal(strategyDailyPoints('senior',i),50);assert.equal(strategyDailyPoints('custom',i),77)});
